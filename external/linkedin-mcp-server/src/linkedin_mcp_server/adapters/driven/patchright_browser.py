...
class PatchrightBrowserAdapter(BrowserPort):
    ...
    async def extract_initial_page_html(self, url: str) -> PageContent:
        """Navigate and extract <main> innerHTML without deep scrolling."""
        page = await self._ensure_browser()
        await self.navigate(url)
        ...
        await self._handle_modal_close(page)
        await self._wait_for_main(page)
        await self._settle_initial_results(page)
    ...
    async def _settle_initial_results(self, page: Page) -> None:
        """Wait briefly for the first visible search result batch."""
        try:
            await page.evaluate("""
                async () => {
                    const delay = ms => new Promise(r => setTimeout(r, ms));
            ...
            logger.debug("Initial result settle error (non-fatal): %s", e)

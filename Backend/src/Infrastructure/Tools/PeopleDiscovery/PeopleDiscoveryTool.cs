...
internal sealed class PeopleDiscoveryTool(IPeopleDiscoveryAdapter adapter) : ITool
{
    public string Name => "people.search";

    public async Task<ToolResult> ExecuteAsync(ToolRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var results = await adapter.SearchAsync(request.Query, request.UserId, cancellationToken);
            return new ToolResult
            {
                Success = true,
                ToolName = Name,
                Summary = results.Count > 0 ? "Results found." : "No people found for this search.",
                Data = results
            };
        }
        catch (Exception exception)
        {
            ...
            return new ToolResult
            {
                Success = false,
                ToolName = Name,
                Summary = authenticationError ? "LinkedIn session is not authenticated." : "LinkedIn MCP is not available.",
